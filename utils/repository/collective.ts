...
export class CollectiveRepository {
  ...
  public insert(collection: string, model: Collective, result: (error, data) => void): void {
    this.db.collection(collection).insertOne(model, (error, insert) => {
      return result(error, insert.ops[0]);
    });
  }

  public update(collection: string, objectId: string, model: Collective, result: (error, data) => void): void {
  ...
  public updateMember(collection: string, objectId: string, idMember: string, model: Collective,  result: (error, data) => void): void {
    this.db.collection(collection).updateOne(
      { _id: new ObjectID(objectId) },
      { $addToSet: {members: ObjectID(idMember) } },
      (error, update) => result(error, model)
    );
  }
  ...
  public deleteMember(collection: string, idMember: string, model: Collective,  result: (error, data) => void): void {

    this.db.collection(collection).update(
      { },
      { $pull: { members: { $in: [ObjectID(idMember)] } } },
      { multi: true },
